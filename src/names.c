...
#endif
{
	...
	if (!did_one) {
		margin_printf(wrap_state ? out_init_struct : outfile, "/* Table of constant values */\n\n");
		did_one = 1;
		}
	...
	if (litp->littype == TYCHAR) {
		...
		if (wrap_state)
		{
			nice_printf(out_init_struct,
				"struct { %s fill; char val[%ld+1];", halign, x);
			nice_printf(out_init_struct, " char fill2[%ld];", hsize - 1);
			nice_printf(out_init_struct, " } %s_st;", litname);
    ...
    if (did_one && !wrap_state)
    	nice_printf (outfile, "\n");
} /* wr_globals */

/* We write an accessor outside the function because we only know if the function
 * needs an accessor after writing it!
 * So it's much easier to write a function that returns an empty value than to 
 * write a pre-pass into the software.
 * Any function that doesn't need an accessor just uses a stub state typedef
 * to keep the compiler satisfied. */
 void
#ifdef KR_headers
wr_state_accessor(outfile)
	FILE *outfile;
#else
wr_state_accessor(FILE *outfile)
#endif
{
	if (!wrap_state)
		return;

	int has_initialized_state = ftell(out_init_struct);
	int has_uninitialized_state = ftell(out_uninit_struct);

	/* Dummy typedef to keep the stub codepath happy. */
	if (!has_initialized_state && !has_uninitialized_state)
	{
