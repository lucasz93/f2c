...
/* DON'T want to use oneof_stg() here; need to find the right common name
   */
	...
	nice_printf (fp, "__state->%s", namep->cvarname);
	else
...
/* Now run through the named arguments */
	...
	if (wrap_state && !is_c_function(extsymtab[name->addrblock.memno].cextname))
